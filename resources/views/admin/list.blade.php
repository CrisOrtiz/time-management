...
<script>
    ...
    window.getSortedNameUsersUrl = "{!! route('admin.name.sorted.get') !!}";
    window.getSortedUserTypeUsersUrl = "{!! route('admin.usertype.sorted.get') !!}";
    window.registerUserUrl = "{!! route('admin.register') !!}";
    window.deleteUserUrl = "{!! route('admin.delete.user') !!}";
    window.updateUserUrl = "{!! route('admin.update.user') !!}";
    window.tasks = @json($tasks);
    window.user = @json($user);
    window.notes = @json($notes);
    window.users = @json($users);
</script>
<script src="{{ mix('js/admin/list.js') }}"></script>
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table table-md table-stripped table-bordered" id="tasks-table" style="text-align:center;">
                            <thead style="color:white; background-color:#007BFF">
                                <tr>
                                    <th scope="col" style="width:20%">Nombres <i v-on:click="onChangeNameSort()" style="color:white" v-bind:class="{'fa fa-sort-up ml-2': namesort=='asc',  'fa fa-sort-down ml-2': namesort=='desc'}" aria-hidden="true"></i></th>
                                    <th scope="col" style="width:20%">Apellidos</th>
                                    <th scope="col" style="width:20%">Usuario</th>
                                    <th scope="col" style="width:20%">Tipo de usuario <i v-on:click="onChangeUserTypeSort()" style="color:white" v-bind:class="{'fa fa-sort-up ml-2': usertypesort=='asc',  'fa fa-sort-down ml-2': usertypesort=='desc'}" aria-hidden="true"></i></th>
                            ...
                            <paginate name="users" :list="users" :per="showNumber">
                                    <tr v-for="u in  paginated('users')">
                                    <td>@{{ u.name }}</td>
                                    <td>@{{ u.surname }}</td>
                                    <td>@{{ u.username }}</td>
                                    <td v-if="u.user_type == 1">Administrador</td>
                                    <td v-if="u.user_type == 2">Manager</td>
                                    <td v-if="u.user_type == 3">Usuario</td>
                                    <td class="d-flex justify-content-md-arround" colspan="2">
                                        <button class="btn btn-primary btn-edit btn-md m-1" type="button" v-on:click="showEditUserModal(u);"><i class="fas fa-edit"></i></button>
                                        <button class="btn btn-danger btn-delete btn-md m-1" type="button" v-on:click="confirmUserDelete(u);"><i class="fas fa-trash"></i></button>
                                        <button v-if="u.user_type == 3" class="btn btn-secondary btn-delete btn-md m-1" type="button" v-on:click="viewTasks(u);"><i class="fas fa-clipboard"></i></button>
                                    </td>
                                </tr>
                            </tbody>
                            <tfoot>
